...
- (void)initSubviews {
    ...
    [_changeValueButton setTitle:@"changeValue"
                          forState:UIControlStateNormal];
    _changeValueButton.backgroundColor = [UIColor redColor];
    [self.view addSubview:_changeValueButton];
    [_changeValueButton addTarget:self action:@selector(changeKey1) forControlEvents:UIControlEventTouchUpInside];
}
...
//    语法 keyPath 的值为keypathToCollection.@collectionOperator.keypathToProperty
...
- (void)objectOperation {
   NSArray *arr = @[@12,@13,@14,@14,@15,@100];
    NSArray *distinctArr = [arr valueForKeyPath:@"@distinctUnionOfObjects.doubleValue"];
...
- (void)addChildObserver {
    [_child addObserver:self forKeyPath:@"key_1" options:NSKeyValueObservingOptionNew | NSKeyValueObservingOptionOld context:NULL];
}

#pragma mark implementation
- (void)observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary<NSString *,id> *)change context:(void *)context {
    if ([keyPath isEqual:@"key_1"]) {
        NSLog(@"oldValue = %@,newValue = %@",[change objectForKey:NSKeyValueChangeOldKey],[change objectForKey:NSKeyValueChangeNewKey]);
    }
